...
const dataFile = require('../../../api/v1/emailRemindersSMTPFile.js')

const configDataFile = path.join(__dirname, '..', '..', '..', 'data', 'emailRemindersSMTP.json')
const sampleData = {
  host: 'localhost',
  port: 587,
  user: 'username',
    ...
    context('when the file does not exist', () => {
      ...
      context('when creating the file fails', () => {
        ...
        it('returns a 500 error', () => {
          ...
          expect(fs.writeFileSync).to.be.calledWith(configDataFile, JSON.stringify({ host: '', port: 587, user: 'user', password: 'pass' }), 'utf-8')
        })
      })

      context('when creating the file succeeds', () => {
        it('succeeds', () => {
          expect(JSON.stringify(dataFile.readData())).to.equal(JSON.stringify({ host: '', port: 587, user: 'user', password: 'pass' }))
          expect(fs.readFileSync).to.be.calledWith(configDataFile, 'utf-8')
          expect(fs.writeFileSync).to.be.calledWith(configDataFile, JSON.stringify({ host: '', port: 587, user: 'user', password: 'pass' }), 'utf-8')
        })
      })
